...
qa:
	...
	docker-compose run --rm php-lowest rm -rf tests/bundle/* var/cache/
	...
	docker-compose run --rm -e SYMFONY_DEPRECATIONS_HELPER="max[direct]=0" php-lowest php vendor/bin/phpunit
	docker-compose run --rm php-highest php -d memory_limit=-1 /usr/bin/composer update --prefer-dist
	docker-compose run --rm php-highest rm -rf tests/bundle/* var/cache/
