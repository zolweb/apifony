...
class TimeValidator implements File
{
    ...
    public function getContent(): string
    {
        ...
        $validateMethod = $f->method('validate')
            ...
            ->addStmt(new If_($f->funcCall('\is_string', [$f->var('value')]), ['stmts' => [
                new Expression(new Assign($f->var('normalizedValue'), $f->funcCall('str_replace', [new Array_([new ArrayItem($f->val('z'))], ['kind' => Array_::KIND_SHORT]), new Array_([new ArrayItem($f->val('Z'))], ['kind' => Array_::KIND_SHORT]), $f->var('value')]))),
                new Expression(new Assign($f->var('allowedFormats'), new Array_([new ArrayItem($f->val('!H:i:sP')), new ArrayItem($f->val('!H:i:s\Z')), new ArrayItem($f->val('!H:i:s.uP')), new ArrayItem($f->val('!H:i:s.u\Z'))], ['kind' => Array_::KIND_SHORT]))),
                ...
                new Expression($f->methodCall($f->methodCall($f->propertyFetch($f->var('this'), 'context'), 'buildViolation', [$f->val('This is not a valid time format according to RFC 3339.')]), 'addViolation')),
            ]]))
        ;

        $class = $f->class('TimeValidator')
