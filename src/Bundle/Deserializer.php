            ...
            $f->namespace("{$this->bundleNamespace}\\Api")
                ...
                ->addStmt($f->class('Deserializer')->implement('DeserializerInterface')
                    ...
                    ->addStmt($f->method('denormalize')
                        ...
                        ->addParam($f->param('type')->setType('string'))
                        ->setReturnType('object')
                        ->addStmt(new Return_($f->methodCall($f->propertyFetch($f->var('this'), 'serializer'), 'denormalize', $f->args([$f->var('data'), $f->var('type'), $f->val(null), new Array_([new ArrayItem($f->val(true), $f->classConstFetch('AbstractNormalizer', 'REQUIRE_ALL_PROPERTIES')), new ArrayItem($f->val(true), $f->classConstFetch('AbstractObjectNormalizer', 'DISABLE_TYPE_ENFORCEMENT'))])]))))
                    )
                )
                ->getNode(),
        ]);
